...
/// <summary>
/// Which thread count is in force, and — the half that makes it reviewable —
/// where it came from.
///
/// <para>"CPU, 4 threads (benchmark 2026-08-16)" and "CPU, 4 threads (20% of 20
/// logical processors)" are the same number arrived at two different ways, and
/// only one of them is a measurement. <c>config</c> reports the reason for that
/// reason.</para>
/// </summary>
...
public sealed record CpuProfileDecision(int Threads, string Provider, string Reason, bool FromProfile)
{
    /// <summary>
    /// Apply a stored profile if it still describes this machine, and fall back
    /// to <see cref="CpuBudget"/>'s percentage otherwise.
    ///
    ...
    /// curve is not monotonic, which is the entire reason this phase exists.
    /// Falling back to the percentage is a guess, but it is the guess the product
    /// already shipped, and it is labelled as one.</para>
    /// </summary>
    public static CpuProfileDecision Decide(
        BenchmarkProfile? stored, BenchmarkMachine now, int maxCpuPercent, int processorCount)
    {
        ArgumentNullException.ThrowIfNull(now);

        if (stored is not null && stored.StalenessAgainst(now).Count == 0)
        {
            string measured = stored.MeasuredUtc.Length >= 10 ? stored.MeasuredUtc[..10] : stored.MeasuredUtc;
            return new CpuProfileDecision(
                stored.Threads, stored.Provider, $"benchmark {measured}", FromProfile: true);
        }

        int threads = CpuBudget.IntraOpThreads(maxCpuPercent, processorCount);
        string why = stored is null
        ...
        return new CpuProfileDecision(threads, "cpu", why, FromProfile: false);
    }

    /// <summary>How the thread count reads in a log line or a status field.</summary>
